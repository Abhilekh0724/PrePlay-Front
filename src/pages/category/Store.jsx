...
const Store = () => {
    ...
    const fetchDownloadProducts = async () => {
      try {
        ...
        if (response.data.success) {
          const downloadGames = response.data.data.filter(
            (product) => product.mode === "download"
          ); // Filter for "download" mode
          setGames(downloadGames);
        }
      } catch (error) {
  ...
  if (games.length === 0) {
    return <div style={{ color: "#fff", textAlign: "center", marginTop: "50px" }}>No games available for download.</div>;
  }
